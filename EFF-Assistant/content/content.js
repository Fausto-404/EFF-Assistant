(() => {
  ...
  function renderSmartHoverAction(hint, disabled) {
    ...
    // 点击 Smart 入口即直接执行，此处仅展示功能说明
    let tipBody = `
      <header class="detached-head" data-role="panel-drag"><div><strong>智能研判</strong></div><div class="detached-actions"></div></header>
    ...
    if (state.busy.smart) {
      ...
      tipBody = `
        <header class="detached-head" data-role="panel-drag"><div><strong>研判中</strong></div><div class="detached-actions"></div></header>
  ...
  function renderQuickHoverAction(hint, disabled) {
    ...
    let tipBody = `
      <header class="detached-head" data-role="panel-drag"><div><strong>点击快速研判</strong></div><div class="detached-actions"></div></header>
      <div class="quick-inline-summary">${escapeHtml(hint)}</div>
    `;
  ...
  function renderDetachedActionTipBody(panel) {
    ...
    if (panel.status === "running") {
      ...
      return `
        <header class="detached-head" data-role="panel-drag"><div><strong>${label}</strong></div><div class="detached-actions">${actions}</div></header>
        <div class="detached-body"><div class="quick-inline-loading"><b></b><span>${escapeHtml(panel.loadingText || "分析中…")}</span></div></div>
      `;
    }
    if (panel.status === "error") {
      return `
        <header class="detached-head" data-role="panel-drag"><div><strong>${label}</strong></div><div class="detached-actions">${actions}</div></header>
        <div class="detached-body"><div class="quick-inline-summary error-text">${escapeHtml(panel.error || "分析失败")}</div></div>
      `;
    }
  ...
  function renderSmartDetachedResult(panel) {
    ...
    const actions = renderPanelActions(panel, true, true);
    return `
      <header class="detached-head" data-role="panel-drag"><div><strong>智能研判</strong></div><div class="detached-actions">${actions}</div></header>
      <div class="detached-body"><div class="quick-inline-verdict"><span class="quick-inline-label">研判结论</span><div class="quick-inline-tags">${chips}</div></div>
      <div class="quick-inline-meta">${view.meta.map((item) => `<span>${escapeHtml(item)}</span>`).join("")}</div>
      <div class="quick-inline-summary">${escapeHtml(view.summary || "当前证据不足，无法形成稳定结论。")}</div>
      <div class="quick-inline-metrics">
        <span>置信度 <b>${escapeHtml(view.confidence)}</b></span>
  ...
  function renderQuickDetachedResult(panel) {
    ...
    const actions = renderPanelActions(panel, true, true);
    return `
      <header class="detached-head" data-role="panel-drag"><div><strong>快速研判</strong></div><div class="detached-actions">${actions}</div></header>
      <div class="detached-body"><div class="quick-inline-verdict"><span class="quick-inline-label">研判结论</span><div class="quick-inline-tags">${chips}</div></div>
      ${meta.length ? `<div class="quick-inline-meta">${meta.join("")}</div>` : ""}
      <div class="quick-inline-summary">${escapeHtml(result.summary || result.conclusion || "当前证据不足，无法形成稳定结论。")}</div>
      <div class="quick-inline-metrics">
  ...
  function normalizeTicketTags(ctx) {
    ...
    push(ctx?.name);
    push(criticalityLabel[ctx?.criticality] || (typeof ctx?.criticality === "string" ? ctx.criticality : "") || (typeof ctx?.importance === "string" ? ctx.importance : ""));
    push(typeof ctx?.area === "string" ? ctx.area : "");
    if (ctx?.owner) push(`负责人:${ctx.owner}`);
    for (const tag of (ctx?.tags || [])) push(typeof tag === "string" ? tag : (tag?.label ? String(tag.label) : ""));
    for (const tag of (ctx?.labels || [])) push(typeof tag === "string" ? tag : (tag?.label ? String(tag.label) : ""));
  ...
  function bindDockEvents() {
    ...
    shadow.querySelectorAll('[data-act="open-action"]').forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        if (action === "smart" && !state.analysisResult && !state.busy.smart) {
          await analyze("smart");
          return;
        }
        if (action === "correlate") {
  ...
  function bindActionTipDetach(tip) {
    ...
    tip.addEventListener("pointerdown", (e) => {
      ...
      if (!e.target.closest(".detached-head")) return;
  ...
  function styles() {
    return `
      ...
      *{box-sizing:border-box}
      ...
      /* 浅色模式 Ticket 面板 */
      .shell.light .ticket-tip.interactive{background:linear-gradient(180deg,rgba(250,253,255,.98),rgba(238,246,253,.98));border-color:rgba(88,139,198,.38);color:#24364b}
      .shell.light .ticket-card-head strong{color:#213248!important}
      .shell.light .ticket-state{background:rgba(106,125,148,.10);color:#597089}
      .shell.light .ticket-state.ok{color:#177b5e;background:rgba(35,165,114,.10)}
      .shell.light .ticket-state.running{color:#6347c7;background:rgba(118,87,232,.10)}
      .shell.light .ticket-kv{background:rgba(240,247,253,.60);border-color:rgba(116,150,188,.20)}
      .shell.light .ticket-kv span{color:#637891!important}
      .shell.light .ticket-kv b{color:#24364b}
      .shell.light .ticket-ip-tags em{background:rgba(230,242,250,.70);border-color:rgba(96,166,235,.22);color:#386ea8}
      .shell.light .ticket-ip-tags em.tag-blacklist,.shell.light .ticket-tags em.tag-blacklist{color:#b84050;background:linear-gradient(135deg,rgba(222,57,80,.12),rgba(222,57,80,.06));border-color:rgba(222,57,80,.28)}
      .shell.light .ticket-ip-tags em.tag-whitelist,.shell.light .ticket-tags em.tag-whitelist{color:#177b5e;background:linear-gradient(135deg,rgba(35,165,114,.10),rgba(35,165,114,.04));border-color:rgba(35,165,114,.24)}
      .shell.light .ticket-ip-tags em.tag-critical,.shell.light .ticket-tags em.tag-critical{color:#9b6822;background:rgba(216,145,45,.10);border-color:rgba(216,145,45,.28)}
      .shell.light .ticket-template select{background:#f8fbff;border-color:#c7d8ea;color:#25364b}
      .shell.light .ticket-hash{background:rgba(35,165,114,.06);border-color:rgba(35,165,114,.18);color:#177b5e}
      .shell.light .ticket-actions button{background:#eef5fc;border-color:#c1d3e8;color:#315b80}
      .shell.light .ticket-actions button:first-child{background:linear-gradient(135deg,#2f83d8,#4072c8);border-color:#6ba4df;color:#fff}
      /* 浅色模式关联分析 + 证据项 */
      .shell.light .quick-inline-verdict{background:rgba(242,248,253,.70);border-color:rgba(116,150,188,.18)}
      .shell.light .quick-inline-empty{color:#7a8ea5}
      .shell.light .quick-inline-reason{border-bottom-color:rgba(116,150,188,.14)}
      .shell.light .quick-inline-reason strong{color:#24364b!important}
      .shell.light .quick-inline-reason span{color:#637891!important}
      .shell.light .correlate-tip.interactive{background:linear-gradient(180deg,rgba(250,253,255,.98),rgba(238,246,253,.98));border-color:rgba(88,139,198,.38);color:#24364b}
      .shell.light .correlate-row{background:rgba(240,247,253,.55);border-color:rgba(116,150,188,.20)}
      .shell.light .correlate-row strong{color:#213248!important}
      .shell.light .correlate-row em{color:#637891}
      .shell.light .correlate-row button{background:rgba(222,57,80,.06);border-color:rgba(222,57,80,.22);color:#bf4051}
      .shell.light .correlate-actions button{background:#eef5fc;border-color:#c1d3e8;color:#315b80}
      .shell.light .correlate-actions button:first-child{background:linear-gradient(135deg,#2f83d8,#4072c8);border-color:#6ba4df;color:#fff}
      /* 浅色模式完成态绿色光晕 */
