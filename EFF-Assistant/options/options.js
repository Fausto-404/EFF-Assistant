...
async function connectPlatform() {
  ...
  try {
    ...
    const test = await send({ type: "TEST_PLATFORM", baseUrl, token });
...
async function syncDevices() {
  ...
  try {
    ...
    const r2 = await send({ type: "SYNC_DEVICES" });
...
async function testQuickAI() {
  ...
  try {
    ...
    const r = await send({ type: "TEST_QUICK_AI", config });
...
async function fetchQuickModels() {
  ...
  try {
    ...
    const r = await send({ type: "LIST_QUICK_AI_MODELS", config });
...
function send(message) { return new Promise((resolve) => chrome.runtime.sendMessage(message, (r) => resolve(r || { ok: false, error: { message: chrome.runtime.lastError?.message || "插件后台无响应" } }))); }
