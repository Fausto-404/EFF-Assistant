...
async function handleMessage(message, sender) {
  switch (message?.type) {
    case "GET_STATE": {
      ...
      return {
        ...
        quickAI: {
          ...
          configured: !!(cfg.quickAI?.baseUrl && cfg.quickAI?.apiKey && cfg.quickAI?.model),
    ...
    case "SAVE_UI_STATE": {
      ...
      await saveConfig(cfg);
      return { ui: { petPosition: cfg.ui.petPosition || null, assistantEnabled: cfg.ui.assistantEnabled !== false } };
    }
    case "TEST_PLATFORM": {
    ...
    case "REFRESH_ACTIVE_TAB": {
      ...
      const cfg = await getConfig();
      const device = detectDeviceForUrl(tab.url || "", cfg.devices || []);
      if (!device) throw new Error("当前 URL 未匹配启用浏览器助手的设备");
      await ensureContentScript(tab.id);
      const result = await sendTabMessage(tab.id, { type: "REFRESH_PARSE" }, 5000);
      return { widget: result, device };
    }
...
async function refreshActiveTabAfterConfigSync(cfg) {
  ...
  try {
    ...
    if (detectDeviceForUrl(tab.url || "", cfg.devices || [])) {
      await sendTabMessage(tab.id, { type: "REFRESH_PARSE" }, 6000);
    }
  } catch (e) {
...
function applyPlatformMeta(target, data) {
  ...
  target.workspace = data.workspace?.name || data.workspace_name || data.workspace || "";
...
async function callQuickAI(config, rawText, isTest, page = {}) {
  ...
  if (!quick.baseUrl || !quick.apiKey || !quick.model) throw new Error("快速 AI 配置不完整");
...
async function testQuickAIConnection(config) {
  ...
  if (!quick.baseUrl || !quick.apiKey || !quick.model) throw new Error("测试前请填写 API 地址、API Key 和模型");
...
async function listQuickAIModels(config) {
  const quick = normalizeQuickProvider(config);
  if (!quick.baseUrl || !quick.apiKey) throw new Error("获取模型前请填写 API 地址和 API Key");
...
async function requestJsonAcrossRoutes({ label, cacheKey, routes, initForRoute, purpose }) {
  ...
  for (const route of ordered) {
    ...
    try {
      const resp = await fetch(route.url, { ...initForRoute(route), cache: "no-store" });
...
function providerHeaders(quick, authMode, protocol) {
  ...
  if (effectiveAuth === "x-api-key") headers["x-api-key"] = quick.apiKey;
  else if (effectiveAuth === "api-key") headers["api-key"] = quick.apiKey;
  else headers.Authorization = `Bearer ${quick.apiKey}`;
  return headers;
}
...
function normalizeAnalysisResult(data) {
  ...
  return {
    ...
    scene_confidence: Number.isFinite(Number(d.scene_confidence ?? data?.scene_confidence)) ? Math.max(0, Math.min(100, Number(d.scene_confidence ?? data?.scene_confidence))) : null,
    ...
    confidence: Number.isFinite(Number(d.confidence)) ? Math.max(0, Math.min(100, Number(d.confidence))) : null,
...
const QUICK_VERDICT_LABELS = ["业务行为", "存在攻击意图", "攻击失败", "攻击成功", "非告警事件", "无法确认"];
